...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.found_address_base = ""
        self.found_postal_code = ""
        self.include_zip = False
        ...
        self.zip_check = self.findChild(QCheckBox, "includeZipCheckBox")
        if self.zip_check:
            self.zip_check.stateChanged.connect(self.on_zip_toggled)
    ...
    def on_zip_toggled(self, state):
        self.include_zip = (state == Qt.CheckState.Checked.value)
        self.update_address_display()

    def update_address_display(self):
        if not self.found_address_base:
            self.address_label.setText("Введите адрес для поиска")
            return

        text = f"{self.found_address_base}"
        if self.include_zip and self.found_postal_code:
            text += f", индекс: {self.found_postal_code}"
    ...
    def _extract_address(self, root):
        try:
            ...
            if formatted is not None and formatted.text:
                addr = formatted.text.strip()
                if addr.endswith(self.found_postal_code) if self.found_postal_code else False:
                    addr = addr.replace(f", {self.found_postal_code}", "").strip()
                return addr
    ...
    def _extract_postal_code(self, root):
        try:
            # Яндекс может отдавать индекс в разных тегах
    ...
    def search_object(self):
        ...
        try:
            ...
            self.found_address_base = self._extract_address(root)

            self.update_address_display()

            self._set_map_position(lon, lat)
            self.statusBar().showMessage(f"Найдено: {self.found_address_base[:40]}...")
