...
class ClickableLabel(QLabel):
    ...
    def mousePressEvent(self, event: QMouseEvent):
        if event.button() == Qt.MouseButton.LeftButton:
            ...
            lon, lat = self.pixel_to_coords(click_x, click_y)
            if self.parent_window:
                self.parent_window.search_organization_by_click(lon, lat)
            event.accept()
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.statusBar().showMessage("Готово. ЛКМ - адрес, ПКМ - организация.")
        self.refresh_map()
    ...
    def _extract_address(self, root):
        try:
            ...
            if formatted is not None and formatted.text: return formatted.text.strip()
            text_elem = root.find(".//GeocoderMetaData/text")
            if text_elem is not None and text_elem.text: return text_elem.text.strip()
    ...
    def _extract_postal_code(self, root):
        try:
            for path in [".//postal_code", ".//AddressComponent[kind='postal_code']/name"]:
                elem = root.find(path)
                if elem is not None and elem.text and elem.text.strip(): return elem.text.strip()
        except Exception:
            pass
        return ""
    ...
    def search_object(self):
        ...
        geocoder_params = {"geocode": query, "apikey": API_KEY_GEOCODER, "format": "xml", "lang": "ru_RU", "results": 1}
        try:
            ...
            pos_elem = root.find(".//Point/pos") or root.find(".//pos")
            ...
            display_text = f"📍 {self.found_address}" + (
                f", индекс: {self.found_postal_code}" if self.found_postal_code else "")
            if self.address_label: self.address_label.setText(display_text)
    ...
    def search_by_click(self, lon, lat):
        self.statusBar().showMessage(f"Поиск адреса: {lon:.6f}, {lat:.6f}...")
        self.search_point = None
        self.found_address = ""
        self.found_postal_code = ""

        geocoder_params = {"geocode": f"{lon},{lat}", "apikey": API_KEY_GEOCODER, "format": "xml", "lang": "ru_RU",
                           "results": 1}
        try:
            response = requests.get("https://geocode-maps.yandex.ru/1.x/", params=geocoder_params, timeout=10)
            if response.status_code != 200: raise Exception(f"HTTP {response.status_code}")
            ...
            if pos_elem is None or not pos_elem.text or not pos_elem.text.strip():
                ...
                self.refresh_map()
                self.statusBar().showMessage("ℹТочный адрес не найден")
                return
    ...
    def search_organization_by_click(self, click_lon, click_lat):
        self.statusBar().showMessage("Поиск организации...")
        ...
        try:
            resp = requests.get("https://geocode-maps.yandex.ru/1.x/", params=params, timeout=10)
            ...
            geo_objects = root.findall(".//GeoObject")
            if not geo_objects:
                self.statusBar().showMessage("Объекты не найдены")
                return
            ...
            for geo_obj in geo_objects:
                pos_elem = geo_obj.find(".//Point/pos")
                if pos_elem is None:
                    pos_elem = geo_obj.find(".//pos")
                ...
                obj_lon, obj_lat = float(coords[0]), float(coords[1])
                distance = self.haversine_distance(click_lat, click_lon, obj_lat, obj_lon)

                meta = geo_obj.find(".//GeocoderMetaData/kind")
                is_biz = meta is not None and meta.text and 'biz' in meta.text.lower()

                if not is_biz:
                    name_elem = geo_obj.find("name")
                    desc_elem = geo_obj.find("description")
                    name = name_elem.text.lower() if name_elem is not None and name_elem.text else ""
                    desc = desc_elem.text.lower() if desc_elem is not None and desc_elem.text else ""
                    biz_keywords = ['магазин', 'кафе', 'ресторан', 'офис', 'компания', 'аптека', 'банк', 'салон',
                                    'клиника']
                    is_biz = any(kw in desc or kw in name for kw in biz_keywords)

                if is_biz and distance <= 50.0:
                    found_org = geo_obj
                    found_distance = distance
                    break

            if found_org is not None and found_distance is not None:
                org_name_elem = found_org.find("name")
                org_name = org_name_elem.text.strip() if org_name_elem is not None and org_name_elem.text else "Организация"

                org_text_elem = found_org.find(".//GeocoderMetaData/text")
                org_addr = org_text_elem.text.strip() if org_text_elem is not None and org_text_elem.text else ""

                display_info = f"{org_name}" + (f" | {org_addr}" if org_addr else "")

                pos_elem = found_org.find(".//Point/pos")
                if pos_elem is None:
                    pos_elem = found_org.find(".//pos")
                if pos_elem is not None and pos_elem.text:
                    org_coords = pos_elem.text.strip().split()
                    if len(org_coords) == 2:
                        org_lon, org_lat = float(org_coords[0]), float(org_coords[1])
                        self.search_point = [org_lon, org_lat]

                self.found_address = display_info
                if self.address_label:
                    self.address_label.setText(display_info)

                self.refresh_map()
                self.statusBar().showMessage(f"Найдено: {org_name} ({found_distance:.0f}м)")
            else:
                self.statusBar().showMessage("Организации в радиусе 50м не найдены")
